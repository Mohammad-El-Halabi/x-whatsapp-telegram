...
export function useSupabase() {
  const api = useMemo(() => ({
    ...
    getStaffAssignments: (userId: string, platform: string = 'whatsapp') =>
      invoke<any[]>('supabase_get_staff_assignments', { userId, platform }),
    getUser: (userId: string) =>
      invoke<any[]>('supabase_get_user', { userId }),
    getClients: (officeId: string) =>
      invoke<any[]>('supabase_get_clients', { officeId }),
    getAllowedContacts: (officeId: string, platform: string, slotId: string, gatewayNumber: string) =>
      invoke<any[]>('supabase_get_allowed_contacts', { officeId, platform, slotId, gatewayNumber }),
